...
export function apiPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      app.get('/api/reports/sales-by-plu', async (req, res) => {
        try {
          const format = (req.query.format as string) || 'html';
          const storeId = req.query.store_id as string;
          const dateFrom = (req.query.date_from as string) || '';
          ...
          for (const tx of (txns as any[]) ?? []) {
            for (const si of tx.sale_items ?? []) {
              ...
              ent.txSet.add(tx.transaction_id);
          ...
          let pluMap = new Map<string, string>();
          if (pluIds.length) {
          ...
          const rows = Array.from(map.values())
            .map((v) => ({
              plu_number: v.plu_id ? pluMap.get(v.plu_id) || '—' : '—',
              plu_name: v.plu_name,
          ...
          if (format === 'csv') {
            ...
            return res.set({ 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=sales-by-plu.csv' }).send(csv);
          }
      ...
      // ---- Reports: loyalty ----
      app.get('/api/reports/loyalty', async (req, res) => {
        try {
          const format = (req.query.format as string) || 'html';
          const storeId = req.query.store_id as string;
          const dateFrom = (req.query.date_from as string) || '';
          ...
          if (format === 'csv') {
            ...
            return res.set({ 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=loyalty.csv' }).send(csv);
          }
          if (format === 'pdf') return res.status(501).json({ error: 'PDF export not yet implemented' });
        ...
        try {
          ...
          const rows = ((data as any[]) ?? []).map((ts: any) => {
            ...
            if (clockIn && clockOut) {
              ...
              const grossH = diffMs / 3600000;
              if (grossH > 6) breakMins = 30; else if (grossH > 4) breakMins = 15;
              const netH = Math.max(0, grossH - breakMins / 60);
              hours = Number(netH.toFixed(2));
              overtime = netH > 8 ? Number((netH - 8).toFixed(2)) : 0;
            } else if (clockIn && !clockOut) flag = 'missing punch';
            return {
              date: clockIn ? clockIn.toISOString().slice(0, 10) : '—',
              user: ts.users?.full_name || ts.user_id?.slice(0, 8) || '—',
              store: ts.stores?.name || ts.store_id?.slice(0, 8) || '—',
              clock_in: clockIn ? clockIn.toISOString().slice(0, 19).replace('T', ' ') : '—',
          ...
          if (format === 'csv') {
            const csv = ['date,user,store,clock_in,clock_out,break_mins,hours,overtime,flag', ...rows.map((r) => `${r.date},"${r.user}","${r.store}",${r.clock_in},${r.clock_out},${r.break_mins},${r.hours},${r.overtime},${r.flag}`)].join('\n');
            return res.set({ 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=timesheets.csv' }).send(csv);
          }
          if (format === 'pdf') return res.status(501).json({ error: 'PDF export not yet implemented' });
      ...
      app.get('/api/reports/stock', async (req, res) => {
        try {
          ...
          const { data, error } = await q;
          if (error) return res.status(400).json({ error: error.message });
          const rows = ((data as any[]) ?? []).map((inv: any) => {
            const pluNum = inv.plu?.plu_number || '—';
            const pluName = inv.plu?.name || inv.name || '—';
            const category = inv.plu?.plu_categories?.name || 'Uncategorised';
            const qty = Number(inv.stock_quantity) || 0;
            const price = Number(inv.price) || 0;
            const value = round2(qty * price);
            const low = qty <= 5;
            return { plu_number: pluNum, plu_name: pluName, category, store: inv.stores?.name || inv.store_id?.slice(0, 8) || '—', stock_quantity: qty, price, stock_value: value, low_stock: low ? 'YES' : 'NO' };
          });
          if (format === 'csv') {
            const csv = ['plu_number,plu_name,category,store,stock_quantity,price,stock_value,low_stock', ...rows.map((r) => `${r.plu_number},"${r.plu_name}","${r.category}","${r.store}",${r.stock_quantity},${r.price},${r.stock_value},${r.low_stock}`)].join('\n');
            return res.set({ 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=stock.csv' }).send(csv);
          }
          if (format === 'pdf') return res.status(501).json({ error: 'PDF export not yet implemented' });
      ...
      // ---- Reports: cogs (FIFO) ----
      app.get('/api/reports/cogs', async (req, res) => {
        try {
          const format = (req.query.format as string) || 'html';
          const storeId = req.query.store_id as string;
          const dateFrom = (req.query.date_from as string) || '';
          ...
          const { buildLayers, fifoCogs } = await import('../lib/fifo');
          const layersMap = buildLayers(((poItems as any[]) ?? []).map((r: any) => ({ plu_id: r.plu_id, quantity_received: Number(r.quantity_received) || 0, cost_price_at_order: Number(r.cost_price_at_order) || 0, delivered_date: r.purchase_orders?.delivered_date, received_at: r.purchase_orders?.received_at })));
          // Sales in range
          ...
          // clone layers for consumption
          const mutable = new Map<string, any[]>();
          ...
          for (const tx of (txns as any[]) ?? []) {
            for (const si of tx.sale_items ?? []) {
              ...
              let cogs = 0;
              if (layers.length && si.plu_id) {
                const res = fifoCogs(layers, qty);
                cogs = res.cogs;
                mutable.set(si.plu_id, res.remaining);
              } else {
                // no FIFO layers, fallback to 0 cogs
                cogs = 0;
              }
          ...
          // Need plu_number for display
          const pluIds = Array.from(agg.keys()).filter((k) => /^[0-9a-f-]{36}$/.test(k));
          let pluNumMap = new Map<string, string>();
          if (pluIds.length) {
            const { data: plus } = await supabaseAdmin.from('plu').select('plu_id, plu_number').in('plu_id', pluIds);
            for (const p of (plus as any[]) ?? []) pluNumMap.set(p.plu_id, p.plu_number);
          }
          const rows = Array.from(agg.entries()).map(([key, v]) => {
            const margin = round2(v.revenue - v.cogs);
            const marginPct = v.revenue ? Number(((margin / v.revenue) * 100).toFixed(1)) : 0;
            return { plu_number: pluNumMap.get(key) || (key.length === 36 ? '—' : key), plu_name: v.plu_name, quantity: Number(v.qty.toFixed(3)), revenue: round2(v.revenue), cogs: round2(v.cogs), margin, margin_pct: marginPct };
          }).sort((a, b) => b.revenue - a.revenue);
          if (format === 'csv') {
            const csv = ['plu_number,plu_name,quantity,revenue,cogs,margin,margin_pct', ...rows.map((r) => `${r.plu_number},"${r.plu_name}",${r.quantity},${r.revenue},${r.cogs},${r.margin},${r.margin_pct}`)].join('\n');
            return res.set({ 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=cogs.csv' }).send(csv);
          }
          if (format === 'pdf') return res.status(501).json({ error: 'PDF export not yet implemented' });
      ...
      // ---- Reports: goods-in ----
      app.get('/api/reports/goods-in', async (req, res) => {
        try {
          const format = (req.query.format as string) || 'html';
          const storeId = req.query.store_id as string;
          const dateFrom = (req.query.date_from as string) || '';
          ...
          for (const po of (data as any[]) ?? []) {
            for (const poi of po.purchase_order_items ?? []) {
              ...
              rows.push({
                po_number: po.po_number,
                supplier: po.suppliers?.name || '—',
                store: po.stores?.name || '—',
                plu_number: poi.plu?.plu_number || '—',
                plu_name: poi.plu?.name || '—',
                ordered,
                received,
                shortage: Number((ordered - received).toFixed(3)),
                status: po.status,
                delivered_date: po.delivered_date || (po.received_at ? new Date(po.received_at).toISOString().slice(0, 10) : '—'),
              });
            }
            if (!(po.purchase_order_items ?? []).length) {
              rows.push({ po_number: po.po_number, supplier: po.suppliers?.name || '—', store: po.stores?.name || '—', plu_number: '—', plu_name: '—', ordered: 0, received: 0, shortage: 0, status: po.status, delivered_date: po.delivered_date || '—' });
            }
          }
          if (format === 'csv') {
            const csv = ['po_number,supplier,store,plu_number,plu_name,ordered,received,shortage,status,delivered_date', ...rows.map((r) => `${r.po_number},"${r.supplier}","${r.store}",${r.plu_number},"${r.plu_name}",${r.ordered},${r.received},${r.shortage},${r.status},${r.delivered_date}`)].join('\n');
            return res.set({ 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=goods-in.csv' }).send(csv);
          }
          if (format === 'pdf') return res.status(501).json({ error: 'PDF export not yet implemented' });
      ...
      // ---- Reports: plu-list ----
      app.get('/api/reports/plu-list', async (req, res) => {
        try {
          const format = (req.query.format as string) || 'html';
          const supabaseAdmin = getSupabaseAdmin(server);
          ...
          const rows = ((data as any[]) ?? []).map((p: any) => ({
            plu_number: p.plu_number,
            name: p.name,
            category: p.plu_categories?.name || 'Uncategorised',
            ean: p.ean || '—',
            uses_scale: p.uses_scale ? 'YES' : 'NO',
            vat_class: p.vat_class,
            headoffice_price: p.headoffice_price,
          }));
          if (format === 'csv') {
            const csv = ['plu_number,name,category,ean,uses_scale,vat_class,headoffice_price', ...rows.map((r) => `${r.plu_number},"${r.name}","${r.category}",${r.ean},${r.uses_scale},${r.vat_class},${r.headoffice_price}`)].join('\n');
            return res.set({ 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=plu-list.csv' }).send(csv);
          }
          if (format === 'pdf') return res.status(501).json({ error: 'PDF export not yet implemented' });
      ...
      app.get('/api/reports/users-stores', async (req, res) => {
        try {
          const format = (req.query.format as string) || 'html';
          const supabaseAdmin = getSupabaseAdmin(server);
          ...
          const userRows = ((users as any[]) ?? []).map((u: any) => ({ type: 'USER', name: u.full_name, email: u.email || '—', role: u.role, pin: u.pin_hash ? 'YES' : 'NO', store: u.stores?.name || u.assigned_store_id?.slice(0, 8) || '—', active: u.is_active ? 'YES' : 'NO' }));
          ...
          if (format === 'csv') {
            const csv = ['type,name,email,role,pin,store,active', ...rows.map((r) => `${r.type},"${r.name}","${r.email}",${r.role},${r.pin},"${r.store}",${r.active}`)].join('\n');
            return res.set({ 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=users-stores.csv' }).send(csv);
          }
          if (format === 'pdf') return res.status(501).json({ error: 'PDF export not yet implemented' });
