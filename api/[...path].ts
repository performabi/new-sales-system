...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (path[0] === 'pos' && path[1] === 'pin-login' && method === 'POST') {
      const { pin } = body;
      if (!/^\d{4,8}$/.test(String(pin || ''))) {
        return res.status(400).json({ error: 'PIN must be 4-8 digits' });
      }
...
return res.json({ success: true, status: newStatus, delivered_date: effectiveDeliveredDate, ...(receiver ? { received_by: receiver.user_id } : {}) });
    ...
    if (path[0] === 'reports' && path[1] === 'purchase-orders' && method === 'GET') {
      try {
        ...
        if (error) return res.status(400).json({ error: error.message });
        const rows = ((data as any[]) ?? []).map((po: any) => { const items = po.purchase_order_items ?? []; const qtyOrdered = items.reduce((s: number, it: any) => s + (Number(it.quantity_ordered) || 0), 0); const qtyReceived = items.reduce((s: number, it: any) => s + (Number(it.quantity_received) || 0), 0); return { po_number: po.po_number, supplier: po.suppliers?.name || '—', store: po.stores?.name || po.store_id?.slice(0, 8) || '—', status: po.status, total_cost: po.total_cost, qty_ordered: Number(qtyOrdered.toFixed(3)), qty_received: Number(qtyReceived.toFixed(3)), shortage: Number((qtyOrdered - qtyReceived).toFixed(3)), expected_delivery: po.expected_delivery_date || '—', delivered_date: po.delivered_date || '—', created_at: po.created_at ? new Date(po.created_at).toISOString().slice(0, 10) : '—' }; });
        if (format === 'csv') { const csv = ['po_number,supplier,store,status,total_cost,qty_ordered,qty_received,shortage,expected_delivery,delivered_date,created_at', ...rows.map((r) => `${r.po_number},"${r.supplier}","${r.store}",${r.status},${r.total_cost},${r.qty_ordered},${r.qty_received},${r.shortage},${r.expected_delivery},${r.delivered_date},${r.created_at}`)].join('\n'); res.setHeader('Content-Type', 'text/csv'); res.setHeader('Content-Disposition', 'attachment; filename=purchase-orders.csv'); return res.send(csv); }
        if (format === 'pdf') return res.status(501).json({ error: 'PDF export not yet implemented' });
